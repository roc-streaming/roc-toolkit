...
device create --name roc-device-x86_64-linux-android$API --image default --api $API --arch x86_64
device start --name roc-device-x86_64-linux-android$API

adb shell "su 0 mkdir -p ${target_prefix}/lib"
adb push "$ANDROID_NDK_ROOT/toolchains/llvm/prebuilt/linux-x86_64/sysroot/usr/lib/x86_64-linux-android/libc++_shared.so" \
        $target_prefix/lib/libc++_shared.so 1> /dev/null

readarray -d '' tests < <(find bin/x86_64-linux-android$API \
