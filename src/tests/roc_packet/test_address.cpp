...
TEST(address, set_ipv4) {
    ...
    CHECK(addr.set_host_ipv4("1.2.0.255", 123));
    CHECK(addr.valid());
...
TEST(address, eq_ipv4) {
    Address addr1;
    CHECK(addr1.set_host_ipv4("1.2.3.4", 123));
    CHECK(addr1.valid());

    Address addr2;
    CHECK(addr2.set_host_ipv4("1.2.3.4", 123));
    CHECK(addr2.valid());

    Address addr3;
    CHECK(addr3.set_host_ipv4("1.2.3.4", 456));
    CHECK(addr3.valid());

    Address addr4;
    CHECK(addr4.set_host_ipv4("1.2.4.3", 123));
    CHECK(addr4.valid());
...
TEST(address, eq_ipv4_multicast) {
    ...
    Address addr3;
    CHECK(addr3.set_host_ipv4("1.2.3.4", 123));
...
TEST(address, eq_ipv6) {
    Address addr1;
    CHECK(addr1.set_host_ipv6("2001:db1::1", 123));
    CHECK(addr1.valid());

    Address addr2;
    CHECK(addr2.set_host_ipv6("2001:db1::1", 123));
    CHECK(addr2.valid());

    Address addr3;
    CHECK(addr3.set_host_ipv6("2001:db1::1", 456));
    CHECK(addr3.valid());

    Address addr4;
    CHECK(addr4.set_host_ipv6("2001:db2::1", 123));
    CHECK(addr4.valid());
...
TEST(address, eq_ipv6_multicast) {
    Address addr1;
    CHECK(addr1.set_host_ipv6("ffee::", 123));
    CHECK(addr1.set_miface_ipv6("::"));
    CHECK(addr1.valid());
    CHECK(addr1.multicast());

    Address addr2;
    CHECK(addr2.set_host_ipv6("ffee::", 123));
    CHECK(addr2.set_miface_ipv6("::"));
    CHECK(addr2.valid());
    CHECK(addr1.multicast());

    Address addr3;
    CHECK(addr3.set_host_ipv6("ffee::", 123));
...
TEST(address, multicast_ipv4) {
    {
        Address addr;
        CHECK(addr.set_host_ipv4("223.255.255.255", 123));
        CHECK(addr.valid());
    ...
    {
        Address addr;
        CHECK(addr.set_host_ipv4("224.0.0.0", 123));
        CHECK(addr.valid());
        CHECK(addr.multicast());
    }
    ...
    {
        Address addr;
        CHECK(addr.set_host_ipv4("227.128.128.128", 123));
        CHECK(addr.valid());
        CHECK(addr.multicast());
    }
    ...
    {
        Address addr;
        CHECK(addr.set_host_ipv4("239.255.255.255", 123));
        CHECK(addr.valid());
        CHECK(addr.multicast());
    }
    ...
    {
        Address addr;
        CHECK(addr.set_host_ipv4("240.0.0.0", 123));
        CHECK(addr.valid());
        CHECK(!addr.multicast());
    }
}
...
TEST(address, multicast_ipv6) {
    {
        Address addr;
        CHECK(addr.set_host_ipv6("fe00::", 123));
        CHECK(addr.valid());
        CHECK(!addr.multicast());
    }
    ...
    {
        Address addr;
        CHECK(addr.set_host_ipv6("ff00::", 123));
        CHECK(addr.valid());
        CHECK(addr.multicast());
    }
    ...
    {
        Address addr;
        CHECK(addr.set_host_ipv6("ff11:1:1:1:1:1:1:1", 123));
        CHECK(addr.valid());
        CHECK(addr.multicast());
    }
    ...
    {
        Address addr;
        CHECK(addr.set_host_ipv6("ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff", 123));
        CHECK(addr.valid());
        CHECK(addr.multicast());
    }
}
