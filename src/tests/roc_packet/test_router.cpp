...
TEST(router, no_routes) {
    ...
    CHECK_EQUAL(status::StatusOK, router.write(p));
...
TEST(router, one_route) {
    ...
    CHECK_EQUAL(status::StatusOK, router.write(wpa1));
    CHECK_EQUAL(status::StatusOK, router.write(wpa2));

    CHECK_EQUAL(status::StatusOK, router.write(pf1));
    CHECK_EQUAL(status::StatusOK, router.write(pf2));
    ...
    PacketPtr rpa1;
    PacketPtr rpa2;
    CHECK_EQUAL(status::StatusOK, queue.read(rpa1));
    CHECK_EQUAL(status::StatusOK, queue.read(rpa2));
    CHECK(wpa1 == rpa1);
    CHECK(wpa2 == rpa2);
...
TEST(router, two_routes) {
    ...
    CHECK_EQUAL(status::StatusOK, router.write(wpr1));
    CHECK_EQUAL(status::StatusOK, router.write(wpr2));
    ...
    PacketPtr rpa1;
    PacketPtr rpa2;
    CHECK_EQUAL(status::StatusOK, queue_a.read(rpa1));
    CHECK_EQUAL(status::StatusOK, queue_a.read(rpa2));
    CHECK(wpa1 == rpa1);
    CHECK(wpa2 == rpa2);

    PacketPtr rpr1;
    PacketPtr rpr2;
    CHECK_EQUAL(status::StatusOK, queue_r.read(rpr1));
    CHECK_EQUAL(status::StatusOK, queue_r.read(rpr2));
    CHECK(wpr1 == rpr1);
    CHECK(wpr2 == rpr2);

    CHECK_EQUAL(0, queue_a.size());
    CHECK_EQUAL(0, queue_r.size());
}
...
TEST(router, same_route_different_sources) {
    ...
    CHECK_EQUAL(status::StatusOK, router.write(new_rtp_packet(11, Packet::FlagAudio)));
    CHECK_EQUAL(1, queue.size());

    // Dropped because have different source.
    CHECK_EQUAL(status::StatusOK, router.write(new_rtp_packet(22, Packet::FlagAudio)));
    CHECK_EQUAL(1, queue.size());
...
TEST(router, different_routes_same_source) {
    ...
    CHECK_EQUAL(status::StatusOK, router.write(new_rtp_packet(11, Packet::FlagAudio)));
    CHECK_EQUAL(status::StatusOK, router.write(new_rtp_packet(11, Packet::FlagRepair)));

    CHECK_EQUAL(1, queue_a.size());
    CHECK_EQUAL(1, queue_r.size());
}
...
TEST(router, different_routes_different_sources) {
    ...
    CHECK_EQUAL(status::StatusOK, router.write(new_rtp_packet(11, Packet::FlagAudio)));
    CHECK_EQUAL(status::StatusOK, router.write(new_rtp_packet(22, Packet::FlagRepair)));
...
TEST(router, same_route_first_without_source_then_with_source) {
    ...
    CHECK_EQUAL(status::StatusOK, router.write(new_fec_packet(Packet::FlagRepair)));
    CHECK_EQUAL(1, queue.size());
...
TEST(router, source_id_one_source) {
    ...
    CHECK_EQUAL(0, router.get_source_id(Packet::FlagAudio));
    CHECK_EQUAL(0, router.get_source_id(Packet::FlagRepair));
    ...
    CHECK(router.has_source_id(Packet::FlagAudio));
    CHECK(!router.has_source_id(Packet::FlagRepair));
    CHECK_EQUAL(11, router.get_source_id(Packet::FlagAudio));
    CHECK_EQUAL(0, router.get_source_id(Packet::FlagRepair));
}
...
TEST(router, source_id_two_sources) {
    ...
    CHECK(router.has_source_id(Packet::FlagRepair));
    CHECK_EQUAL(11, router.get_source_id(Packet::FlagAudio));
    CHECK_EQUAL(22, router.get_source_id(Packet::FlagRepair));
}
