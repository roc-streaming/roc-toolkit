...
TEST(socket_addr, eq_ipv4_multicast) {
    SocketAddr addr1;
    CHECK(addr1.set_host_port(Family_IPv4, "1.2.3.4", 123));
    CHECK(addr1.set_miface(Family_IPv4, "0.0.0.0"));
    CHECK(addr1.has_host_port());

    SocketAddr addr2;
    CHECK(addr2.set_host_port(Family_IPv4, "1.2.3.4", 123));
    CHECK(addr2.set_miface(Family_IPv4, "0.0.0.0"));
    CHECK(addr2.has_host_port());

    SocketAddr addr3;
    CHECK(addr3.set_host_port(Family_IPv4, "1.2.3.4", 123));
    CHECK(addr3.set_miface(Family_IPv4, "0.0.0.1"));
    CHECK(addr3.has_host_port());
...
TEST(socket_addr, eq_ipv6_multicast) {
    SocketAddr addr1;
    CHECK(addr1.set_host_port(Family_IPv6, "ffee::", 123));
    CHECK(addr1.set_miface(Family_IPv6, "::"));
    CHECK(addr1.has_host_port());
    CHECK(addr1.multicast());

    SocketAddr addr2;
    CHECK(addr2.set_host_port(Family_IPv6, "ffee::", 123));
    CHECK(addr2.set_miface(Family_IPv6, "::"));
    CHECK(addr2.has_host_port());
    CHECK(addr1.multicast());

    SocketAddr addr3;
    CHECK(addr3.set_host_port(Family_IPv6, "ffee::", 123));
    CHECK(addr3.set_miface(Family_IPv6, "2001:db8::1"));
    CHECK(addr3.has_host_port());
    CHECK(addr1.multicast());
...
TEST(socket_addr, multicast_ipv4_to_str) {
    ...
    }
    {
        ...
        CHECK(addr.set_miface(Family_IPv4, "0.0.0.0"));
...
TEST(socket_addr, multicast_ipv6_to_str) {
    ...
    }
    {
        ...
        CHECK(addr.set_miface(Family_IPv6, "::"));
