...
TEST(state_tracker, simple_timeout) {
    ...
    TestThread thr(state_tracker,sndio::DeviceState_Active, core::timestamp(core::ClockMonotonic) + core::Second * 0.1);
...
TEST(state_tracker, multiple_timeout) {
    ...
    for (int i=0;i<10;i++){
      threads_ptr[i] = new TestThread(state_tracker,sndio::DeviceState_Active, core::timestamp(core::ClockMonotonic) + core::Second * 1);
    }
