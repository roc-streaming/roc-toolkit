...
TEST(sndfile_source, noop) {
    SndfileSource sndfile_source(arena, source_config);
}

TEST(sndfile_source, error) {
    SndfileSource sndfile_source(arena, source_config);

    CHECK(!sndfile_source.open(NULL, "/bad/file"));
}

TEST(sndfile_source, has_clock) {
...
TEST(sndfile_source, sample_rate_auto) {
    ...
    CHECK(sndfile_source.open(NULL, file.path()));
    CHECK(sndfile_source.sample_spec().sample_rate() == SampleRate);
}

TEST(sndfile_source, sample_rate_mismatch) {
    core::TempFile file("test.wav");
