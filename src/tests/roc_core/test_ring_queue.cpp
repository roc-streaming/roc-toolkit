...
TEST(ring_queue, is_empty_is_full) {
    RingQueue<Object, EmbeddedCap> queue(arena, NumObjects + 1);
...
TEST(ring_queue, embedding) {
    ...
    LONGS_EQUAL(EmbeddedCap - 1, queue.capacity());
    ...
    // Check queue behavior at capacity minus 1
    LONGS_EQUAL(EmbeddedCap - 1, queue.size());
    CHECK(queue.is_full());
    LONGS_EQUAL(0, queue.front().value);
    LONGS_EQUAL(EmbeddedCap - 2, queue.back().value);
