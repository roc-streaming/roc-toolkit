...
void Depacketizer::read(Frame& frame) {
    ...
    const packet::timestamp_t prev_zero_samples = zero_samples_;
...
void Depacketizer::set_frame_flags_(Frame& frame,
                                    ...
                                    const packet::timestamp_t prev_zero_samples) {
    ...
    if (first_packet_) {
        zero_samples = num_channels_
            * (size_t)ROC_UNSIGNED_SUB(packet::signed_timestamp_t, zero_samples_,
                                       prev_zero_samples);
    } else {
        zero_samples = num_channels_
            * (size_t)ROC_UNSIGNED_SUB(packet::signed_timestamp_t, missing_samples_,
                                       prev_missing_samples);
    }
