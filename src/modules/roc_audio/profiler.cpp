...
Profiler::Profiler(core::IAllocator& allocator,
    ...
    , chunk_length_((size_t)(
          sample_spec.getSampleRate() * ((float)profiler_config.chunk_duration / (float)core::Second)))
    ...
    , valid_(false)
    , buffer_full_(false) {
    if (sample_spec.num_channels() == 0) {
        roc_panic("profiler: n_channels is zero");
    }
    if (sample_spec.getSampleRate() == 0) {
...
void Profiler::add_frame(size_t frame_size, core::nanoseconds_t elapsed) {
    ...
    if (rate_limiter_.allow()) {
        roc_log(LogDebug,
                ...
                (double)get_moving_avg() / sample_spec_.getSampleRate());
    }
}
