...
void Reporter::process_dlrr_subblock(const header::XrPacket& xr,
    ...
    if (stream->last_local_rr != 0 && stream->last_remote_sr != 0
        ...
        if (stream->remote_send_rtt.has_metrics()) {
            ...
            stream->remote_send_report.rtt = metrics.rtt_min;
        }
    }
...
void Reporter::process_rrtr_block(const header::XrPacket& xr,
    ...
    if (stream->last_local_sr != 0 && stream->last_remote_rr != 0
        ...
        if (stream->remote_recv_rtt.has_metrics()) {
            ...
            stream->remote_recv_report.rtt = metrics.rtt_min;
        }
    }
...
void Reporter::generate_measurement_info_block(size_t addr_index,
    ...
    blk.set_ssrc(stream->source_id);
    blk.set_first_sn((packet::seqnum_t)stream->local_recv_report->ext_first_seqnum);

    if (stream->remote_send_rtt.has_metrics()) {
        // If RTT metrics are available, define measurement interval according to them.
        const RttMetrics& metrics = stream->remote_send_rtt.metrics();

        blk.set_interval_first_sn(metrics.interval_first_seqnum);
        blk.set_interval_last_sn(metrics.interval_last_seqnum);
        blk.set_interval_duration(packet::nanoseconds_2_ntp(metrics.interval_duration));
        blk.set_cum_duration(packet::nanoseconds_2_ntp(metrics.cumulative_duration));
    } else {
        // Otherwise, define measurement interval as instant point.
        blk.set_interval_first_sn(stream->local_recv_report->ext_last_seqnum);
        blk.set_interval_last_sn(stream->local_recv_report->ext_last_seqnum);
...
void Reporter::generate_delay_metrics_block(size_t addr_index,
    ...
    blk.set_ssrc(stream->source_id);
    blk.set_metric_flag(header::MetricFlag_IntervalDuration);

    if (stream->remote_send_rtt.has_metrics()) {
        const RttMetrics& metrics = stream->remote_send_rtt.metrics();

        blk.set_mean_rtt(packet::nanoseconds_2_ntp(metrics.rtt_avg));
        blk.set_min_rtt(packet::nanoseconds_2_ntp(metrics.rtt_min));
        blk.set_max_rtt(packet::nanoseconds_2_ntp(metrics.rtt_max));
    }
