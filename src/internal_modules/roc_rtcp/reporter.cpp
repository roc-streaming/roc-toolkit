...
void Reporter::generate_reception_block(size_t addr_index,
    ...
    if (can_generate_last_ts_(stream->last_remote_sr)) {
...
void Reporter::generate_dlrr_subblock(size_t addr_index,
    ...
    if (can_generate_last_ts_(stream->last_remote_rr)) {
...
bool Reporter::can_generate_last_ts_(core::nanoseconds_t last_ts) const {
    if (last_ts <= 0) {
        // We didn't receive SR/RR yet.
        return false;
    }

    if (last_ts > report_time_) {
        // DLSR/DLRR that we're going to generate would be negative,
        // this should not happen.
        return false;
    }

    if (report_time_ - last_ts > max_delay_) {
        // DLSR/DLRR that we're going to generate would overflow,
        // this should not happen.
        return false;
    }
